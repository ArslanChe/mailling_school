...
class DataBase:
    ...
    def read_messages(self, messenger_id):
        self.cursor.execute(f"SELECT user_id FROM user_information WHERE id_{self.messenger} = {messenger_id}")
        user_id = self.cursor.fetchone()
        if user_id is None:
            return False
        user_id = user_id[0]
        #print('||||', user_id)
        self.cursor.execute(f"UPDATE users_messages SET isread = true WHERE user_id = {user_id}")
        self.connect.commit()
        return True
...
print(db.get_messenger_user_id_in_group(group_id))
'''
